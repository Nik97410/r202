...
if oui_non "$0 : Voulez-vous creer un équivalent root pour mysql oui/non ? " ; then
	mysql_equivroot () {
		echo -n "$0: - Donner le login (compte �quivalent root) : "
	    read X_LOGINBDD_X
		echo -n "$0: - Donner le mot de passe (compte �quivalent root) : "
	    read X_PASSWD_X
		...
		if [ $? -eq 0 ]; then
			echo "$0: Cr�ation du compte �quivalent root :"
			echo -e "$0: Login : ${X_LOGINBDD_X}\n Password : ${X_PASSWD_X}"
	    else
	        echo "========= La cr�ation d'un compte a �quivalent root �chou� ========="
	    fi
	}
fi
...
if oui_non "$0 : Voulez-vous modifier le LOGIN d'un compte mysql oui/non ? " ; then
		...
		mysql_modif_login () {
			...
			if [ $? -eq 0 ]; then
		        ...
		        echo "========= La modification du login a �chou� ========="
		    fi
		}
		...
		mysql_modif_password () {
			...
			if [ $? -eq 0 ]; then
		        ...
		        echo "========= La modification du mot de passe a �chou� ========="
		    fi
		}
        ...
        echo "Modification du NOM de la BDD"
				mysql_modif_bddname() {
					...
					if [ $? -eq 0 ]; then
				        ...
				        echo "========= La modification du nom de la BDD a �chou� ========="
				    fi
				}
	fi
fi
...
if oui_non "$0 : Voulez-vous supprimer une BDD mysql oui/non ? " ; then
	mysql_delete_bdd () {
		...
		if [ $? -eq 0 ]; then
			echo "$0: la BDD ${X_DBNAME_X} a bien �t� supprim�e"
	    	else
	        	echo "========= La modification du login a �chou� ========="
	    	fi
	}
...
if oui_non "$0 : Voulez-vous supprimer un compte (LOGIN) d'une BDD mysql oui/non ? " ; then
	mysql_delete_user () {
		...
		if [ $? -eq 0 ]; then
			echo "$0: l'utilisateur (username ou login) ${X_DBNAME_X} a bien �t� supprim�e"
	    	else
	        	echo "========= La modification du login a �chou� ========="
	    	fi
